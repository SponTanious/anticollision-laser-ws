...
public:
  OrientusNode(ros::NodeHandle nh, ros::NodeHandle pnh)
    ...
    pnh_.param("frame_id", frame_id_, std::string("laser"));
    ...
    // Setup ROS topic
    //imu_orient_ = nh.advertise<geometry_msgs::Vector3>("imu/orient", 10);
...
private:
  ...
  /* void publish_orient_msg() {
    sensor_msgs::Imu imu_msg;
    geometry_msgs::Vector3 orientation_message;

    imu_msg.orientation_message.x = raw_sensors_packet_.gyroscopes[0];
    imu_msg.orientation_message.y = raw_sensors_packet_.gyroscopes[1];
    imu_msg.orientation_message.z = raw_sensors_packet_.gyroscopes[2];
  } */
